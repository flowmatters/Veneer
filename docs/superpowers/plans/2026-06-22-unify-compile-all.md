...
- **Latent edge (resolve_in_worktree + absolute paths):** `resolve_in_worktree(wt, p)` is `os.path.normpath(os.path.join(wt, p))`. If `--refpath`/`--source`/solution are ever passed as ABSOLUTE paths, `os.path.join` discards `wt` and every worktree shares one refs/output dir — breaking per-worktree isolation (groups would corrupt each other's references). All defaults and the CI invocation are relative, so this is latent, not active. If absolute paths become a use case, add a guard (reject or warn) or document the constraint.
